 ...
 */
typedef enum { SUL = 1, SUDESTE = 2, NORTE = 3, NORDESTE = 4 } Regiao;
 ...
 *  - Esta funcao deve preencher um vetor estatico interno (local ao arquivo
 ...
 *
 *  > Dica: Dentro de produtos.c, declare: `static Produto catalogo[6];`
 *          Na função, faça: `catalogo[0].id = 1; strcpy(catalogo[0].nome, "Item
 * 1"); ...`
 */
 ...
 *
 * > Dica: O parametro é um ponteiro (`Carrinho *c`) para que a alteracao
 * persista fora da funcao. Use a seta (`->`) para acessar os campos:
 * `c->qtdProdutos = 0;`
 */
void inicializarCarrinho(Carrinho *c);

/**
 * @brief Adiciona um produto ao carrinho ou incrementa sua quantidade se ja
 * existir.
 *
 * DEVE SER IMPLEMENTADO EM: carrinho.c
 *
 ...
 *         - Se houver espaco, adiciona o novo produto na posicao
 ...
 *
 * > Dica: Use um loop `for` de 0 até `c->qtdProdutos` para ver se o item ja
 * existe. Se não existir, verifique `if (c->qtdProdutos < MAX_ITENS)` antes de
 * adicionar.
 */
 ...
 *  - Lista cada item: Nome, Preco Unitario, Quantidade, Subtotal do item (Preco
 * * Qtd).
 ...
 *
 * > Dica: Aqui o parametro é `Carrinho c` (sem ponteiro) pois apenas leremos os
 * dados, sem modificar. Acesse com ponto: `c.itens[i].produto.nome`.
 */
...
/**
 * @brief Calcula o peso total de todos os produtos no carrinho (para calculo de
 * frete).
 *
 ...
 *
 * > Dica: Use `if/else` ou `switch` para checar a `regiao` e dentro de cada
 ...
 *         Se `f == NULL`, deu erro ao criar. Se nao, use `fprintf(f,
